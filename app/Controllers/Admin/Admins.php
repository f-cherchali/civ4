...
class Admins extends GcController{
    public function index(){
        ...
        $crud->callbackAfterUpdate([$this,"_update_session"]);
        $crud->callbackUpload([$this,"_callbackUpload"]);
        $crud->callbackBeforeInsert([$this,"_callback_insert"]);
        $crud->setRule("email","email");
        if($crud->getState()==="Insert"){
            \Valitron\Validator::addRule('checkUniqueEmail', function($field, $value, array $params, array $fields) {
                $db = $this->db;
                $builder = $db->table("admin");
                $query = $builder->where("email",$value)->get();
                if($builder->countAllResults()==0){
                    return true;
                }
                return false;
            }, 'doit être une adresse email unique. Cet adresse est déja utilisée');
            $crud->setRule('email', 'checkUniqueEmail');
        }
        ...
        $crud->fieldType("active","dropdown",[0=>"Desactivé",1=>"Activé"]);
        $crud->unsetDelete();
        $output = $crud->render();
    ...
    function _callbackUpload($uploadData){
    ...
    function _update_session($stateParameters){
        if($stateParameters->primaryKeyValue == $this->session->admin_id){
            $this->session->set("admin_photo",$stateParameters->data['photo']);
        }
        return true;
    }
    ...
    function _callback_insert($stateParameters){
        $stateParameters->data['password']=password_hash("123456",PASSWORD_DEFAULT);
